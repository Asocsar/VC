...
key = 1;
for i = 1:2:mH
    Hists3{key} = hist3(cat(2,Hists{i}(:,1), Hists{i+1}(:,1)), [5,5]);
    key = key + 1;
end
...
%    R = historiogramaBins(crops{1}, NBINS, Hists);    
    for equips = 1:7      
        ...
        for k1 = 1:cs:c
            ...
            %imshow(M);
            R = max(R, uint8(historiogramaBins(M, NBINS, Hists3)));            
            end
        end        
        EKIP(equips) = R;
    end
...
B = imhist(B, 25);
B = HistNorm(B);
histograma = hist3(cat(2,R,B), [5,5]);

mean = 0;
threshold = 8;%0.17;%6;%0.2;
% threshold2 = 4;
% thresholdB = 0.01;
% thresholdW = 0.01;
for j = 1:m
...
%     dist = pdist2aaaa(histograma', Hists{j}','emd');
    dist = pdist2aaaa(histograma', Hists{j}','emd');
    mm = max(max(dist));
    dist = sum(sum(dist))/mm;
